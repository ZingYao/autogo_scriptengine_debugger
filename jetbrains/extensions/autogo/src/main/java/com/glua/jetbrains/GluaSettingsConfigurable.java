...
import com.autogo.jetbrains.AutoGoToolPathResolver;
...
public final class GluaSettingsConfigurable implements Configurable {
    ...
    public void reset() {
        GluaSettings settings = settings();
        // 空路径先自动发现并立即写回配置，只有确实找不到时才保持为空。
        if (settings.gluaExecutable().isBlank()) {
            settings.setGluaExecutable(AutoGoToolPathResolver.findGlua(System.getenv()));
        }
        if (settings.gluacExecutable().isBlank()) {
            String discovered = AutoGoToolPathResolver.findGluac(System.getenv());
            if (discovered.isBlank()) {
                try {
                    // 系统未安装时提取插件内置编译器，设置页直接显示当前实际使用路径。
                    discovered = GluaCompilerExecutable.resolve("").toString();
                } catch (IOException ignored) {
                    // 不支持的平台保持为空，允许用户浏览选择自定义编译器。
                }
            }
            settings.setGluacExecutable(discovered);
        }
        if (settings.languageServerExecutable().isBlank()) {
            settings.setLanguageServerExecutable(AutoGoToolPathResolver.findGluals(System.getenv()));
        }
