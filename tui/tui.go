...
type TUI struct {
	...
	topStatusBar    *tview.TextView // 顶部状态栏（显示状态信息）
	bottomStatusBar *tview.TextView // 底部状态栏（显示快捷键提示）
...
// NewTUI 创建 TUI 界面
func NewTUI(cfg *config.CombinedConfig, configMgr *config.Manager, deviceMgr *device.Manager, projectMgr *project.Manager, p *printer.Printer) *TUI {
	tui := &TUI{
		...
		printer:        p,
		agManager:      agmanager.NewAGManager(p, cfg.Global.AGPath, cfg.Global.HTTPProxy),
		ready:          false,
		selectedScript: cfg.Project.SelectedScript, // 从配置加载选中的脚本
	...
	// 创建统一日志管理器
	tui.logger = NewLogger(tui.app, tui.logView, tui.projectLogView)
...
func (t *TUI) setupUI() {
	...
	// 创建顶部状态栏（显示状态信息）
	t.topStatusBar = tview.NewTextView().
		SetDynamicColors(true).
		SetTextAlign(tview.AlignLeft).
		SetText("[yellow]状态:[white] 就绪")

	// 创建底部状态栏（显示快捷键提示）
	t.bottomStatusBar = tview.NewTextView().
		SetDynamicColors(true).
	...
	t.flex = tview.NewFlex().
		...
		AddItem(t.topStatusBar, 1, 0, false).   // 顶部状态栏固定高度
		AddItem(menuFrame, 12, 0, true).        // 菜单固定 12 行
		AddItem(t.logFlex, 0, 1, false).        // 日志占剩余空间
		AddItem(t.bottomStatusBar, 1, 0, false) // 底部状态栏固定高度
...
// updateStatusBar 更新底部状态栏提示（快捷键提示）
func (t *TUI) updateStatusBar(mode string) {
	switch mode {
	case "menu":
		t.bottomStatusBar.SetText("[yellow]快捷键:[white] 1-4/h/q 选择菜单 | L/D 查看日志 | Tab 切换日志 | Ctrl+数字 快速执行 | R 刷新 | F9 鼠标 | Ctrl+Q 退出")
	case "debugLog":
		t.bottomStatusBar.SetText("[yellow]操作提示:[white] ↑↓滚动 | PgUp/PgDn翻页 | Home/End 跳转首尾 | Tab 切换日志 | Enter 全屏 | R 刷新 | C 清空 | ESC 返回菜单")
	case "projectLog":
		t.bottomStatusBar.SetText("[yellow]操作提示:[white] ↑↓滚动 | PgUp/PgDn翻页 | Home/End 跳转首尾 | Tab 切换日志 | Enter 全屏 | R 刷新 | C 清空 | ESC 返回菜单")
	case "debugLogFullscreen":
		t.bottomStatusBar.SetText("[yellow]全屏模式:[white] ↑↓滚动 | PgUp/PgDn翻页 | Home/End 跳转首尾 | ESC 退出全屏 | R 刷新 | C 清空")
	case "projectLogFullscreen":
		t.bottomStatusBar.SetText("[yellow]全屏模式:[white] ↑↓滚动 | PgUp/PgDn翻页 | Home/End 跳转首尾 | ESC 退出全屏 | R 刷新 | C 清空")
	case "submenu":
		t.bottomStatusBar.SetText("[yellow]操作提示:[white] ↑↓选择 | Enter 确认 | ESC 返回主菜单")
	default:
		t.bottomStatusBar.SetText("[yellow]快捷键:[white] 1-4/h/q 选择菜单 | L/D 查看日志 | Tab 切换日志 | Ctrl+数字 快速执行 | R 刷新 | F9 鼠标 | Ctrl+Q 退出")
	}
}
...
func (t *TUI) handleGlobalInput(event *tcell.EventKey) *tcell.EventKey {
	...
	case 'l', 'L':
		...
		if t.showProjectLog {
			...
			t.warn("项目日志已隐藏，按 Ctrl+Shift+L 显示")
		}
		return nil
	case 'd', 'D':
		// 切换焦点到调试器日志
...
func (t *TUI) initialize() {
	t.initOnce.Do(func() {
		...
		detectedADBPath, err := t.deviceMgr.DetectADB(t.config.Global.ADBPath)
		if err != nil {
			t.error("错误: %s", err)
			return
		}
		...
		if t.config.Global.ADBPath == "" && detectedADBPath != "" {
			t.config.Global.ADBPath = detectedADBPath
		...
		if err := t.projectMgr.DetectAG(); err != nil {
			...
			t.error("未检测到 AG！")
			t.log("")
			t.warn("AG 是 AutoGo 项目的核心工具，用于编译和部署项目到设备。")
			t.log("")
			t.log("安装方法：")
			t.log("  1. 在主菜单选择 [3[] AG 更新")
			t.log("  2. 选择 [检查更新] 自动下载并安装")
			t.log("")
			t.warn("或手动安装：")
			t.log("  - 下载地址: https://github.com/Dasongzi1366/AutoGo/releases")
			t.log("  - 将 ag 可执行文件放到 PATH 目录或设置环境变量 AUTOGO_AG_PATH")
			t.log("")
		...
		// 检查设备连接
		if t.config.Project.DeviceID == "" {
			t.printer.Verbose("未配置设备")
		} else {
			// 已配置设备，自动检测并更新IP
			t.printer.Verbose("检测已配置设备: %s", t.config.Project.DeviceID)
			...
			if err != nil {
				...
				for _, d := range devices {
					if d.Serial == t.config.Project.DeviceID {
						deviceOnline = true
						break
					}
				}
				...
				if !deviceOnline {
					...
					if err != nil {
						...
						t.printer.Verbose("设备已就绪: %s (%s)", t.config.Project.DeviceID, t.config.Runtime.DeviceIP)

						// 检查服务端口是否可访问（检测设备IP，因为服务运行在设备上）
						t.printer.Verbose("检查服务端口 %s:%s...", t.config.Runtime.DeviceIP, t.config.Project.DevicePort)
						if t.deviceMgr.ScanPort(t.config.Runtime.DeviceIP, t.config.Project.DevicePort, 2*time.Second) {
							t.config.Runtime.DeviceServiceURL = fmt.Sprintf("%s:%s", t.config.Runtime.DeviceIP, t.config.Project.DevicePort)
							t.printer.Verbose("服务已运行: %s", t.config.Runtime.DeviceServiceURL)
...
func (t *TUI) runManagementMenu() {
	...
	// 显示当前选择的脚本
	if t.selectedScript != "" {
		t.logger.MSG("当前选择的脚本: %s", t.selectedScript)
	} else {
		t.warn("未选择脚本，请先选择脚本")
	}
	...
	// 创建运行管理布局（上状态栏，中菜单，下日志，底部快捷键）
	runMgmtFlex := tview.NewFlex().
		SetDirection(tview.FlexRow).
		AddItem(t.topStatusBar, 1, 0, false).   // 顶部状态栏
		AddItem(menu, 12, 0, true).             // 菜单固定 12 行
		AddItem(t.logFlex, 0, 1, false).        // 日志占剩余空间
		AddItem(t.bottomStatusBar, 1, 0, false) // 底部快捷键栏
...
func (t *TUI) selectScript() {
	...
	// 扫描 scripts 目录
	scriptsDir := filepath.Join(t.config.Project.ProjectPath, "scripts")
	...
	if len(scriptFiles) == 0 {
		t.warn("未找到脚本文件 (scripts/*.lua 或 scripts/*.js)")
		return
	}
	...
	form.AddDropDown("脚本文件:", scriptFiles, currentIndex, func(option string, index int) {
		...
		AddButton("确认", func() {
			...
			t.config.Project.SelectedScript = selectedScript
			...
			// 保存到配置文件
			if err := t.configMgr.Save(t.config); err != nil {
				t.error("保存配置失败: %s", err)
			} else {
				t.printer.Verbose("配置已保存")
				t.log("已选择脚本: %s (类型: %s)", selectedScript, scriptType)
			}
...
func (t *TUI) runScript() {
	...
	if t.scriptStatus == "running" {
		t.error("错误: 脚本正在运行中，请先停止当前脚本")
		return
	}
	if t.scriptStatus == "paused" {
		t.warn("提示: 有暂停的脚本，建议先恢复或停止")
	}

	// 检查设备服务地址
	if t.config.Runtime.DeviceServiceURL == "" {
		t.error("错误: 未配置设备服务地址，请先启动项目")
		return
	}

	// 检查设备 IP 和端口
	if t.config.Runtime.DeviceIP == "" || t.config.Project.DevicePort == "" {
		t.error("错误: 未配置设备 IP 或端口")
		return
	}

	// 检测设备端口连通性
	t.printer.Verbose("检测设备服务: %s:%s", t.config.Runtime.DeviceIP, t.config.Project.DevicePort)
	if !t.deviceMgr.CheckScriptEngineService(t.config.Runtime.DeviceIP, t.config.Project.DevicePort, 3*time.Second) {
		t.error("错误: 设备服务 %s:%s 不可访问或未就绪", t.config.Runtime.DeviceIP, t.config.Project.DevicePort)
		t.warn("请确保项目已在设备上启动")
		return
	}
	...
	t.warn("运行脚本: %s", t.selectedScript)
	t.log("脚本类型: %s | 代码风格: %s", scriptType, t.config.Project.CodeStyle)

	runner := script.NewRunner(t.config.Runtime.DeviceServiceURL, t.printer)
	runner.SetDevice(t.config.Project.DeviceID, os.Getenv("AUTOGO_ADB_PATH"))
	...
	go func() {
		err := runner.Run(scriptPath, scriptType, t.config.Project.CodeStyle, true) // 添加 autoCleanup 参数
		t.scriptStatus = "idle"
		t.runningScript = ""
...
func (t *TUI) startProject() {
	...
	// 检查设备是否已连接
	if t.config.Project.DeviceID == "" || t.config.Runtime.DeviceIP == "" {
		t.error("错误: 未连接设备，请先在设备管理中连接设备")
		t.updateStatus("请先连接设备")
		return
	}

	t.warn("设备: %s (%s)", t.config.Project.DeviceID, t.config.Runtime.DeviceIP)
	t.warn("正在启动项目...")
	t.updateStatus("正在启动项目，请稍候...")

	go func() {
		if err := t.projectMgr.RunAsync(t.config.Project.ProjectPath, t.config.Project.DeviceID, false); err != nil {
			t.error("启动失败: %s", err)
			t.updateStatus("启动失败")
			return
		}

		t.log("项目进程已启动，等待服务就绪...")

		// 等待端口就绪（检测设备IP端口，因为AG命令在设备上启动服务）
		if !t.projectMgr.WaitForReady(t.config.Runtime.DeviceIP, t.config.Project.DevicePort, 60) {
			// 检查进程是否还在运行
			if !t.projectMgr.IsRunning() {
				t.error("项目进程异常终止，请检查项目配置")
			} else {
				t.error("服务启动超时，端口 %s 不可访问", t.config.Project.DevicePort)
			}
			t.updateStatus("启动失败")
			return
		}
		...
		t.config.Runtime.DeviceServiceURL = fmt.Sprintf("%s:%s", t.config.Runtime.DeviceIP, t.config.Project.DevicePort)
		t.log("服务已启动: %s", t.config.Runtime.DeviceServiceURL)
...
func (t *TUI) listDevices() {
	...
	go func() {
		...
		if len(devices) == 0 {
			t.warn("未检测到已连接的设备")
			t.warn("提示: 可以通过 TCP/IP 连接设备")
			return
		}

		t.log("检测到 %d 个设备:", len(devices))
		for i, d := range devices {
			// 注意：%d 不要用方括号包裹，否则会被解析为颜色标签
			t.log("  %d. %s (%s)", i+1, d.Serial, d.State)
		}

		// 如果只有一个设备，提示已自动选择
		if len(devices) == 1 {
			t.log("当前设备: %s", devices[0].Serial)
			t.selectDevice(devices[0].Serial)
		} else {
			t.warn("提示: 可以选择 (3) 选择设备 来选择要使用的设备")
		}
	}()
}
...
func (t *TUI) selectDevice(deviceID string) {
	...
	go func() {
		...
		if err != nil {
			t.error("获取设备 IP 失败: %s", err)
			t.warn("提示: 请确保设备已连接 WiFi")
			return
		}

		// 检查IP是否变化
		ipChanged := t.config.Runtime.DeviceIP != ip
		t.config.Project.DeviceID = deviceID
		t.config.Runtime.DeviceIP = ip

		if ipChanged && t.config.Runtime.DeviceIP != "" {
			t.log("设备 IP 已更新: %s", ip)
		} else {
			t.log("设备 IP: %s", ip)
		}

		t.log("设备已配置: %s (%s)", deviceID, ip)
...
func (t *TUI) connectDeviceWithLog(logView *tview.TextView) {
	...
	form.AddButton("连接", func() {
		...
		go func() {
			if err := t.deviceMgr.ConnectDevice(address); err != nil {
				...
				devices, _ := t.deviceMgr.GetConnectedDevices()
				if len(devices) > 0 {
					t.log("检测到设备: %s", devices[0].Serial)
					t.selectDeviceWithLog(devices[0].Serial, logView)
				} else {
					t.error("错误: 未检测到设备")
				}
	...
	// 创建布局：顶部状态栏，表单在上，日志在中，底部快捷键
	flex := tview.NewFlex().
		SetDirection(tview.FlexRow).
		AddItem(t.topStatusBar, 1, 0, false).
		AddItem(form, 7, 0, true).
		AddItem(logView, 0, 1, false).
		AddItem(t.bottomStatusBar, 1, 0, false)
...
func (t *TUI) showConfig() {
	t.log("\n=== 当前配置 ===")
	t.log("代码风格: %s", t.config.Project.CodeStyle)
	t.log("设备服务地址: %s", t.config.Runtime.DeviceServiceURL)
	t.log("设备 ID: %s", t.config.Project.DeviceID)
	t.log("设备 IP: %s", t.config.Runtime.DeviceIP)
	t.log("设备端口: %s", t.config.Project.DevicePort)
	t.log("项目路径: %s", t.config.Project.ProjectPath)
...
func (t *TUI) showConfigWithLog(logView *tview.TextView) {
	t.log("\n[cyan]=== 当前配置 ===[white]")
	t.log("[white]代码风格: %s", t.config.Project.CodeStyle)
	t.log("[white]设备服务地址: %s", t.config.Runtime.DeviceServiceURL)
	t.log("[white]设备 ID: %s", t.config.Project.DeviceID)
	t.log("[white]设备 IP: %s", t.config.Runtime.DeviceIP)
	t.log("[white]设备端口: %s", t.config.Project.DevicePort)
	t.log("[white]项目路径: %s", t.config.Project.ProjectPath)
	t.log("[white]AG 版本: %s", t.agManager.GetCurrentVersion())
	t.log("[white]HTTP 代理: %s", t.config.Global.HTTPProxy)
	t.log("[white]AG 路径: %s", t.agManager.GetAGPath())
}
...
func (t *TUI) showConfigManager() {
	...
	t.log("[white]项目路径: %s", t.config.Project.ProjectPath)
	t.log("[white]HTTP 代理: %s", t.config.Global.HTTPProxy)
	t.log("[white]AG 路径: %s", t.config.Global.AGPath)
	t.log("[white]ADB 路径: %s", t.config.Global.ADBPath)

	// 设置菜单选择处理
	configMenu.SetSelectedFunc(func(index int, name string, secondary string, shortcut rune) {
		switch shortcut {
		case '2':
			t.editConfigItem("代码风格", "codeStyle", t.config.Project.CodeStyle, configLogView)
		case '3':
			t.editConfigItem("设备服务地址", "serverUrl", t.config.Runtime.DeviceServiceURL, configLogView)
		case '4':
			t.editConfigItem("设备 ID", "deviceId", t.config.Project.DeviceID, configLogView)
		case '5':
			t.editConfigItem("设备 IP", "deviceIp", t.config.Runtime.DeviceIP, configLogView)
		case '6':
			t.editConfigItem("设备端口", "devicePort", t.config.Project.DevicePort, configLogView)
		case '7':
			t.editConfigItem("项目路径", "projectPath", t.config.Project.ProjectPath, configLogView)
		case '8':
			t.editConfigItem("HTTP 代理", "httpProxy", t.config.Global.HTTPProxy, configLogView)
		case '9':
			t.editConfigItem("AG 路径", "agPath", t.config.Global.AGPath, configLogView)
		case 'a':
			t.editConfigItem("ADB 路径", "adbPath", t.config.Global.ADBPath, configLogView)
		case 'q':
			t.currentMode = "" // 返回主界面
			t.restoreFocus()
		}
...
func (t *TUI) editConfigItem(name, key, currentValue string, logView *tview.TextView) {
	...
	t.log("[white]当前值: %s", currentValue)
	...
	form.AddButton("保存", func() {
		...
		switch key {
		case "codeStyle":
			t.config.Project.CodeStyle = newValue
		case "serverUrl":
			t.config.Runtime.DeviceServiceURL = newValue
		case "deviceId":
			t.config.Project.DeviceID = newValue
		case "deviceIp":
			t.config.Runtime.DeviceIP = newValue
		case "devicePort":
			t.config.Project.DevicePort = newValue
		case "projectPath":
			t.config.Project.ProjectPath = newValue
		case "httpProxy":
			t.config.Global.HTTPProxy = newValue
			t.agManager.SetHTTPProxy(newValue)
		case "agPath":
			t.config.Global.AGPath = newValue
		case "adbPath":
			t.config.Global.ADBPath = newValue
		}
...
func (t *TUI) showAGManager() {
	...
	agMenu.SetSelectedFunc(func(index int, name string, secondary string, shortcut rune) {
		...
		case 'v':
			...
			t.log("AG 安装路径: %s", t.agManager.GetAGPath())
		case 's':
...
func (t *TUI) checkForUpdatesWithLog(logView *tview.TextView) {
	...
	go func() {
		...
		if !t.agManager.IsInstalled() {
			...
			t.log("发现新版本: v%s (当前: v%s)", latest.Version, current)
		}
...
func (t *TUI) performUpdateWithLog(latest agmanager.VersionInfo, logView *tview.TextView) {
	...
	go func() {
		...
		if err != nil {
			...
			t.log("更新成功: v%s", latest.Version)
			t.log("AG 已安装到: %s", t.agManager.GetAGPath())
		}
	}()
...
func (t *TUI) configureProxyWithLog(logView *tview.TextView) {
	t.log("\n[cyan]=== 配置代理 ===[white]")
	t.log("[white]当前代理: %s", t.config.Global.HTTPProxy)

	// 创建表单
	form := tview.NewForm()
	form.AddInputField("代理地址:", t.config.Global.HTTPProxy, 40, nil, nil)
	form.AddButton("确认", func() {
		proxyAddr := form.GetFormItem(0).(*tview.InputField).GetText()
		t.config.Global.HTTPProxy = proxyAddr
...
// warn 记录警告日志（线程安全）
func (t *TUI) warn(format string, args ...interface{}) {
	if t.logger != nil {
...
// debug 记录调试日志（线程安全）
func (t *TUI) debug(format string, args ...interface{}) {
	if t.logger != nil {
...
// help 记录帮助信息（线程安全）
func (t *TUI) help(format string, args ...interface{}) {
	if t.logger != nil {
		t.logger.Help(format, args...)
	}
}
...
func (t *TUI) projectLog(format string, args ...interface{}) {
	if t.logger != nil {
		t.logger.ProjectInfo(format, args...)
	}
}

// updateStatus 更新顶部状态栏（线程安全）- 显示状态信息
func (t *TUI) updateStatus(text string) {
	go func() {
		t.app.QueueUpdateDraw(func() {
			t.topStatusBar.SetText(text)
		})
	}()
}
...
func (t *TUI) initProject() {
	...
	if err != nil {
		t.error("获取当前目录失败: %s", err)
		t.restoreFocus()
		return
	}

	t.log("当前目录: %s", currentDir)
	...
	form.AddInputField("Module 名称:", defaultModuleName, 40, nil, func(text string) {
		...
		AddButton("初始化", func() {
			...
			if moduleName == "" {
				t.error("错误: Module 名称不能为空")
				return
			}

			t.warn("开始初始化项目...")
			t.log("Module: %s", moduleName)
			t.log("目标平台: %s", targetPlatform)
...
func (t *TUI) doInitProject(projectPath, moduleName, target string) {
	...
	for _, f := range files {
		...
		if name == "go.mod" || name == "main.go" || name == "AutoGo" {
			t.warn("警告: 当前目录已存在项目文件")
			// 不阻止，继续执行
		}
	}

	// 2. 调用 ag init 初始化
	t.log("执行 ag init -t %s...", target)
	if err := t.projectMgr.Init(projectPath, target, t.config.Project.DevicePort); err != nil {
		t.error("AG 初始化失败: %s", err)
		t.restoreFocus()
		return
	}
	t.log("AG 初始化完成")
	...
	if err := t.updateGoModModule(goModPath, moduleName); err != nil {
		t.error("更新 go.mod 失败: %s", err)
		t.warn("请手动修改 go.mod 中的 module 名称")
	} else {
		t.log("go.mod module 已更新为: %s", moduleName)
	}

	// 4. 设置 autogo_scriptengine
	if err := t.setupAutoGoScriptEngine(projectPath); err != nil {
		t.error("设置 autogo_scriptengine 失败: %s", err)
		t.warn("请手动设置 autogo_scriptengine")
	} else {
		t.log("autogo_scriptengine 设置完成")
	}
...
func (t *TUI) setupAutoGoScriptEngine(projectPath string) error {
	...
	if _, err := os.Stat(enginePath); err == nil {
		t.warn("删除现有的 AutoGoScriptEngine 目录...")
	...
	// 2. 克隆仓库
	t.log("克隆 autogo_scriptengine 仓库...")
	t.warn("这可能需要几分钟，请耐心等待...")
	...
	t.log("修改 AutoGoScriptEngine/go.mod...")
...
func (t *TUI) addRequireAndReplace(goModPath, module, replacePath string) error {
	...
	if requireAdded && replaceAdded {
		t.warn("require 和 replace 已存在，跳过")
		return nil
	}
