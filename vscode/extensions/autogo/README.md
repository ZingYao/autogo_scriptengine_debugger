...
VSCode 版本与 IDEA 扩展共享 `.autogo/engine.json`、移动端控制协议、manifest 增量同步和 Lua/GLua/JavaScript DAP 语义。扩展不注册编辑器右键菜单；`AutoGo Script Engine Console` 位于底部 Panel，与 Debug Console、Terminal 等标签处于同一行，主要操作在视图标题栏横向排列，完整入口同时出现在命令面板。
...
- 快速调试当前 Lua/GLua/JavaScript：解析静态依赖闭包、合并 `sync.extraFiles`、提示动态依赖、增量同步、重启隔离引擎、启动移动端 DAP，并双向映射本地/设备源码路径。JavaScript 支持 CommonJS `require`、Promise 风格 `importModule`、断点、单步、变量查看、`evaluate` 和 `setVariable`。
...
测试覆盖 AutoGo 命令、工具发现、设备解析、代理、依赖图、Lua/JavaScript 依赖闭包、manifest、远程安全边界、DAP 路径映射，以及 go-lua-vm 的语法、内置 API、诊断、补全、跳转、格式化与调试适配。
