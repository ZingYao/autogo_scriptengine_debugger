...
function luaBlockScanSource(source) {
  let result = "";
  let index = 0;
  let state = "code";
  while (index < source.length) {
    ...
    if (state === "long-comment" || state === "long-string") {
      if (character === "]" && next === "]") {
        result += "  ";
        index += 2;
        state = "code";
    ...
    if (character === "'" || character === '"') {
      result += " ";
      index += 1;
      state = character === "'" ? "single-string" : "double-string";
      continue;
    }
    result += character;
    index += 1;
...
function hasMatchingBlockCloser(source, offset, repeatBlock = false) {
  const suffix = luaBlockScanSource(source.slice(offset));
  const keywords = suffix.match(/\b(?:function|if|for|while|repeat|switch|end|until)\b/g) || [];
  const expectedClosers = [repeatBlock ? "until" : "end"];
  for (const keyword of keywords) {
    if (keyword === "repeat") {
      expectedClosers.push("until");
      continue;
    }
    if (keyword === "function" || keyword === "if" || keyword === "for" || keyword === "while" || keyword === "switch") {
      expectedClosers.push("end");
      continue;
    }
    if (keyword === expectedClosers[expectedClosers.length - 1]) {
      expectedClosers.pop();
...
function blockEnterExpansion(source, offset) {
  ...
  if (trimmed === "repeat") {
    if (hasMatchingBlockCloser(source, safeOffset, true)) {
      return null;
    }
  ...
  if (
    ...
    if (hasMatchingBlockCloser(source, safeOffset)) {
      return null;
    }
...
async function startAttachDebugSession(attachConfig, outputChannel) {
  try {
    ...
    if (started) {
      if (outputChannel) {
        outputChannel.appendLine(`[glua-dap] 调试会话已启动：${attachConfig.name || "GLua"}`);
        outputChannel.show(true);
      }
      return true;
    }
...
function normalizeDebugConfiguration(folder, config, outputChannel) {
  ...
  next.internalConsoleOptions = "neverOpen";
...
function activate(context, sharedOutputChannel) {
  const outputChannel = sharedOutputChannel || vscode.window.createOutputChannel("glua Language Server");
  outputChannelRef = outputChannel;
  if (!sharedOutputChannel) context.subscriptions.push(outputChannel);
