...
const extension = require("../glua-extension");
...
async function main() {
  ...
  assert(!executedCommands.includes("workbench.panel.repl.view.focus"), "successful debug start must not focus Debug Console");
  ...
  assert.strictEqual(
    extension._test.blockEnterExpansion("tools.hello = function(name)\n  print(name)\n  print('hello,'..name)\nend", 28),
    null,
    "Enter must not insert a duplicate end when the assigned function is already closed",
  );
  assert.strictEqual(
    extension._test.blockEnterExpansion("function hello(name)\n  if name then\n    print('end') -- end\n  end\nend", 20),
    null,
    "block matching must ignore end inside strings and comments and respect nested blocks",
  );
  assert.deepStrictEqual(extension._test.blockEnterExpansion("tools.hello = function(name)\n  print(name)", 28), {
    text: "\n  \nend",
    caretDelta: 3,
  });
  ...
  assert.strictEqual(
    extension._test.blockEnterExpansion("repeat\n  if ready then\n    break\n  end\nuntil done", 6),
    null,
    "repeat matching must allow nested end-delimited blocks",
  );
  ...
  assert.deepStrictEqual(debugConsoleText, [], "script stdout must not go to Debug Console");
  assert(outputLines.includes("hello,zing\n"), "script stdout should stay in AutoGo output");
