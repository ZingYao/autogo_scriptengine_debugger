...
const { resolveGlualsExecutable, resolveGluacExecutable } = require("../gluals-resolver");
...
for (const [directory, executable] of [
  ["darwin-amd64", "gluac"],
  ["darwin-arm64", "gluac"],
  ["windows-amd64", "gluac.exe"],
  ["windows-arm64", "gluac.exe"],
]) {
  const target = path.join(root, "bin", directory, executable);
  fs.writeFileSync(target, "test");
}
...
assert.throws(() => resolveGlualsExecutable(root, "", "freebsd", "amd64"), /not bundled/);
assert.equal(resolveGluacExecutable(root, "", "darwin", "arm64").path, path.join(root, "bin", "darwin-arm64", "gluac"));
assert.equal(resolveGluacExecutable(root, "", "win32", "x64").path, path.join(root, "bin", "windows-amd64", "gluac.exe"));
assert.throws(() => resolveGluacExecutable(root, "", "linux", "amd64"), /not bundled/);
